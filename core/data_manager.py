...
from .constant import DATA_FILE_NAME, DATA_PATH, DEFAULT_CFG, PLUGIN_NAME, RECENT_DYNAMIC_CACHE
...
class DataManager:
    ...
    def is_comment_notified(self, sub_user: str, uid: int, cache_key: str) -> bool:
        sub = self.get_subscription(sub_user, uid)
        if not sub:
            return False
        return sub.is_comment_notified(cache_key)

    async def mark_comment_notified(
        self, sub_user: str, uid: int, cache_key: str, limit: int = 200
    ):
        sub = self.get_subscription(sub_user, uid)
        if not sub:
            return
        sub.mark_comment_notified(cache_key, limit)
        await self.save()

    async def update_comment_count(self, sub_user: str, uid: int, oid: str, count: int):
        sub = self.get_subscription(sub_user, uid)
        if not sub:
            return
        sub.update_comment_count(oid, count)
        sub.last_comment_scan_at = int(__import__("time").time())
        await self.save()
